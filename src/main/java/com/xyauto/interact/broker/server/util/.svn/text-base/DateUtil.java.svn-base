...
public class DateUtil {
    ...
    private static Date addInteger(Date date, int dateType, int amount) {
        ...
        if (date != null) {
            ...
            calendar.setTime(date);
            calendar.set(dateType, year + amount);
    ...
    public static Date timeStamp2Date(Long millisecond, String format) {
        if (millisecond == null || millisecond < Long.valueOf("1000000000000") ) {
            return null;
        }
        if (format == null || format.isEmpty()) {
            format = "yyyy-MM-dd HH:mm:ss";
